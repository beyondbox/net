...
public class SpecificationAdapter extends BaseAdapter {
    ...
    public SpecificationAdapter(BaseActivity context, ArrayList<HashMap<String, Object>> list) {
        super();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            // 获取list_item布局文件的视图
            convertView = LayoutInflater.from(this.mContext).inflate(R.layout.item_lv_specification, null, true);
            holder = new ViewHolder();

            // 获取控件对象
            holder.tvTitle = (TextView) convertView
                    .findViewById(R.id.tv_title);//规格标题
            holder.tvStock = (TextView) convertView.findViewById(R.id.tv_stock);//库存
            // 设置控件集到convertView
        ...
        if (this.mList != null) {
            ...
            String type = (String) this.mList.get(position).get(
                    "type");
            holder.tvTitle.setText(type);//规格名称
            //动态加载标签
            ...
            if (holder.vgPropContents.getChildCount() == 0) {
                ...
                for (int i = 0; i < labels.size(); i++) {
                    ...
                    LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT,
                            LinearLayout.LayoutParams.WRAP_CONTENT);
                    lp.setMargins(0, 0,mContext.dip(mContext,12), mContext.dip(mContext,12));
                    ...
                    LinearLayout.LayoutParams l = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT,
                            LinearLayout.LayoutParams.WRAP_CONTENT);
                    l.setMargins(0, 0,mContext.dip(mContext,30), mContext.dip(mContext,12));
                    textValue.setLayoutParams(l);
                    textValues[i] = textValue;
                    textValues[i].setText(values.get(i));
                    textValues[i].setTag(i);
                    textValues[i].setTextSize(14);
                    textValues[i].setTextColor(mContext.getResources().getColor(R.color.theme_color));

...
public class PersonalReleaseActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_personal_release);
        initBack();
    ...
    private class MyHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            PersonalReleaseActivity activity = (PersonalReleaseActivity) reference.get();
            if (activity == null) {
                return;
            }
            switch (msg.what) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.l_expiry_date://信息时长
                Intent intent = new Intent(PersonalReleaseActivity.this, InformationSelectActivity.class);
                Bundle b = new Bundle();
...
//                        updateLabel();
     ...
     * 检查是否已发布过该目的地的路线
     */
    private void locationCheck(final Intent data) {
        initProgressDialog(false, "");

        RequestParams params = MyHttpClient.getApiParam("service", "oneCheck");
        params.put("uid", getUserID());
        params.put("type", 4);
        params.put("product_id", data.getStringExtra("id"));
